...
- **CostmapLayer**：代价地图插件层的基类，基础于`Layer`、`Costmap2D`
    - `CostmapLayer`
...
- **StaticLayer**：静态层，继承与`CostmapLayer`，接收slam生成的地图，然后把代价添加到代价地图中
    - `onInitialize`：重写函数，在`Layer`的`Initialize`最后调用
        - 调用`getParameters`
        - 创建代价地图订阅`map_sub_ = node->create_subscription`，绑定`incomingMap`
        - 如果`subscribe_to_updates=true`，则创建地图更新订阅`map_update_sub_ = node->create_subscription<map_msgs::msg::OccupancyGridUpdate>(
        map_topic_ + "_updates",...)`，绑定`incomingUpdate`
    - `activate`：
    - `deactivate`：调用`dyn_params_handler_.reset()`
    - `reset`：
    - `isClearable`：是否可清除，静态地图不可清除
