...
例如，在[`Sutskever.Vinyals.Le.2014`](https://zh.d2l.ai/chapter_references/zreferences.html#id160)的设计中，
正是基于这种设计将输入序列的编码信息送入到解码器中来生成输出序列的。
在其他一些设计中[`Cho.Van-Merrienboer.Gulcehre.ea.2014`](https://zh.d2l.ai/chapter_references/zreferences.html#id24)，
如图9.7.1所示，
编码器最终的隐状态在每一个时间步都作为解码器的输入序列的一部分。
类似于《语言模型和数据集》中语言模型的训练，
可以允许标签成为原始的输出序列，
...
$$\mathbf{s}_{t^\prime} = g(y_{t^\prime-1}, \mathbf{c}, \mathbf{s}_{t^\prime-1}).\space\space\space\space\space\text{9.7.3}$$
...
！[](../img/output_seq2seq_13725e_171_1.svg)
...
虽然[`Papineni.Roukos.Ward.ea.2002`](https://zh.d2l.ai/chapter_references/zreferences.html#id118)
...
$$ \exp\left(\min\left(0, 1 - \frac{\mathrm{len}_{\text{label}}}{\mathrm{len}_{\text{pred}}}\right)\right) \prod_{n=1}^k p_n^{1/2^n},\space\space\space\text{9.7.4}$$
...
所以公式 (9.7.4)中乘法项之前的系数用于惩罚较短的预测序列。
