...
![不同分辨率的图像10 x 10, 20 x 20, 40 x 40, 80 x 80, 和 160 x 160 pixels](../img/cat-dog-pixels.png)

不同分辨率的图像10 x 10, 20 x 20, 40 x 40, 80 x 80, 和 160 x 160 pixels

如上图所示，虽然人类很容易以$$160 \times 160$$像素的分辨率识别猫和狗，
...
$$P(A \mid B)$$
是一个*条件分布*（conditional distribution）。
...
$$H$$
来表示感染艾滋病病毒的状态（如果阳性，则为$$1$$，如果阴性，则为$$0$$）。
在下表中列出了这样的条件概率。
...
4. 在上面应用中，第一个测试更准确。为什么不运行第一个测试两次，而是同时运行第一个和第二个测试?

![](../img/cf9d3eae80c8d782b6ddf04fa69af1e03cc859dc.png)