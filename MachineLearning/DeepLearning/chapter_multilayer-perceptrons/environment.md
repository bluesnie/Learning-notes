...
$$\mathop{\mathrm{minimize}}_f \frac{1}{n} \sum_{i=1}^n \beta_i l(f(\mathbf{x}_i), y_i). \space\space\space\space\space \text{(4.9.5)}$$
...
4. 使用权重$$\beta_i$$进行公式(4.9.5)中$$\{(\mathbf{x}_1, y_1), \ldots, (\mathbf{x}_n, y_n)\}$$的训练。
...
使用 上面协变量偏移纠正中相同符号，
$$q$$
和$$p$$中分别是源分布（例如训练时的分布）和目标分布（例如测试时的分布）。
假设标签的分布随时间变化：$$q(y) \neq p(y)$$，
但类别条件分布保持不变：$$q(\mathbf{x} \mid y)=p(\mathbf{x} \mid y)$$
。
如果源分布$$q(y)$$是“错误的”，
我们可以根据公式(4.9.2)中定义的真实风险中的恒等式进行更正：
