...
可微注意力模型`Bahdanau.Cho.Bengio.2014`。
在预测词元时，如果不是所有输入词元都相关，模型将仅对齐（或参与）输入序列中与当前预测相关的部分。这是通过将上下文变量视为注意力集中的输出来实现的。

## 模型

下面描述的Bahdanau注意力模型
将遵循《序列到序列学习（seq2seq）》中的相同符号表达。
这个新的基于注意力的模型与《序列到序列学习（seq2seq）》中的模型相同，
...
注意力权重$$\alpha$$是使用公式 (10.3.2)
所定义的加性注意力打分函数计算的。

与 图9.7.2中的循环神经网络编码器-解码器架构略有不同，
 图10.4.1描述了Bahdanau注意力的架构。
...
![](../img/output_bahdanau-attention_7f08d9_53_1.svg)
...
![](../img/output_bahdanau-attention_7f08d9_87_0.svg)
