...
在《锚框》中，我们以输入图像的每个像素为中心，生成了多个锚框。
...
回想一下，在《图像卷积》中，我们将卷积图层的二维数组输出称为特征图。
...
![](../img/output_multiscale-object-detection_ad7147_30_0.svg)
...
![](../img/output_multiscale-object-detection_ad7147_42_0.svg)
...
![](../img/output_multiscale-object-detection_ad7147_54_0.svg)

## 多尺度检测

既然我们已经生成了多尺度的锚框，我们就将使用它们来检测不同尺度下各种大小的目标。
下面，我们介绍一种基于CNN的多尺度目标检测方法，将在《单发多框检测（SSD）》中实现。

在某种规模上，假设我们有$$c$$张形状为$$h \times w$$的特征图。
使用《多尺度目标检测》中的方法，我们生成了$$hw$$组锚框，其中每组都有$$a$$个中心相同的锚框。
例如，在《多尺度目标检测》实验的第一个尺度上，给定10个（通道数量）$$4 \times 4$$的特征图，我们生成了16组锚框，每组包含3个中心相同的锚框。
...
根据《图像卷积》中对感受野的定义，特征图在相同空间位置的$$c$$个单元在输入图像上的感受野相同：
它们表征了同一感受野内的输入图像信息。
...
在《单发多框检测（SSD）》，我们将通过一个具体的例子来说明它是如何工作的。
...
1. 根据我们在《深度卷积神经网络（AlexNet）》中的讨论，深度神经网络学习图像特征级别抽象层次，随网络深度的增加而升级。在多尺度目标检测中，不同尺度的特征映射是否对应于不同的抽象层次？为什么？
2. 在《多尺度目标检测》中的实验里的第一个尺度（`fmap_w=4, fmap_h=4`）下，生成可能重叠的均匀分布的锚框。
