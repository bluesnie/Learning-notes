...
在 《目标检测和边界框》— 《目标检测数据集》中，我们分别介绍了边界框、锚框、多尺度目标检测和用于目标检测的数据集。
...
回想一下在多尺度目标检测中，通过深度神经网络分层表示图像的多尺度目标检测的设计。
由于接近图13.7.1顶部的多尺度特征图较小，但具有较大的感受野，它们适合检测较少但较大的物体。
...
回忆《网络中的网络（NiN）》一节介绍的使用卷积层的通道来输出类别预测的方法，
...
回想一下，在图13.7.1中，一个较接近顶部的多尺度特征块是用于检测较大目标的，因此需要生成更大的锚框。
...
首先，让我们[**读取**]《目标检测数据集》中描述的(**香蕉检测数据集**)。
...
但是，对于这个回归问题，我们在这里不使用《线性回归中解析解》中描述的平方损失，而是使用$$L_1$$范数损失，即预测值和真实值之差的绝对值。
...
```
![](../img/output_ssd_739e1b_210_1.svg)
...
![](../img/output_ssd_739e1b_246_0.svg)
...
![](../img/output_ssd_739e1b_258_0.svg)

此外，在类别预测时，实验中使用了交叉熵损失：设真实类别$$j$$的预测概率是$$p_j$$，交叉熵损失为$$-\log p_j$$。我们还可以使用焦点损失 `Lin.Goyal.Girshick.ea.2017`。给定超参数$$\gamma > 0$$和$$\alpha > 0$$，此损失的定义为：
...
![](../img/output_ssd_739e1b_270_0.svg)
