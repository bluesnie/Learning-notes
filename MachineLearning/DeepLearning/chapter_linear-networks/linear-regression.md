...
$$\hat{y} = w_1  x_1 + ... + w_d  x_d + b.\space\space\space\space\space(3.1.2)$$

将所有特征放到向量$$\mathbf{x} \in \mathbb{R}^d$$中，
并将所有权重放到向量$$\mathbf{w} \in \mathbb{R}^d$$中，
我们可以用点积形式来简洁地表达模型：

$$\hat{y} = \mathbf{w}^\top \mathbf{x} + b.\space\space\space\space\space(3.1.3)$$
...
$${\hat{\mathbf{y}}} = \mathbf{X} \mathbf{w} + b\space\space\space\space\space(3.1.4)$$
...
$$l^{(i)}(\mathbf{w}, b) = \frac{1}{2} \left(\hat{y}^{(i)} - y^{(i)}\right)^2.\space\space\space\space\space(3.1.5)$$
...
$$\mathbf{w}^* = (\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf{y}.\space\space\space\space\space(3.1.8)$$
...
那么加权线性回归里，目标损失函数就变为了

$$ L = (XB-Y)^{T}\Lambda (XB-Y) \\
=& B^{T}X^{T}\Lambda XB-2Y^{T}\Lambda XB + Y^{T}\Lambda Y $$

同样地，令 $$ L $$ 对 $$ B $$求导如下式：

$$ \frac{\partial L}{\partial B} = 2X^{T} \Lambda XB - 2X^{T}\Lambda Y $$

令 $$ \frac{\partial L}{\partial B} = 0 $$ 可求得:

$$ \hat{B} = (X^{T}\Lambda X)^{-1}X^{T}\Lambda Y  $$

同样地，这里要求矩阵 $$ X^{T}\Lambda X $$ 可逆。

- 三、小结

不难看到，令 $$ \omega _{i} = \frac{1}{n} $$ ，加权线性回归就退化为一般等权重线性回归。

    - i) 从观测误差的角度去看，如果一个样本的 $$ \omega _{i} $$
 越大，则可以允许其偏离回归超平面的程度就越大，对这个点的拟合效果就会越差；
   - ii) 从样本点的重要性角度去看，如果一个样本的  $$ \omega _{i} $$ 越大，则其一点微小的误差在损失函数里会被放大，则可以允许其偏离回归超平面的程度就越小，最后的结果相当于对这个点的拟合效果就会越好。
  
注意，这里的 $$ \omega _{i} $$ 是指方差的占比，如果残差波动率为 $$ s_{i} $$, 则应该有 $$ \omega _{i}\propto s_{i}^{\frac{1}{2} } $$ 。

另外，如果我们使用 WLS 回归取残差的话，那么残差 $$ \gamma $$ 的均值并不为零：

$$ \begin{aligned}
X^{T} \Lambda \Upsilon & =X^{T} \Lambda\left(Y-X\left(X^{T} \Lambda X\right)^{-1} X^{T} \Lambda Y\right) \\
& =X^{T} \Lambda Y-X^{T} \Lambda X\left(X^{T} \Lambda X\right)^{-1} X^{T} \Lambda Y .\\
& = 0
\end{aligned} $$
...
![](../img/afc22815da737ef9f6ce925fb2d4d1367d83aa28.png)

![](../img/16cd28ae6f5f9bd4cf35a770caa8b0cca118ef8e.jpeg)