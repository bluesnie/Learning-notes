...
* 其次，衰减速率同样很重要。如果学习率持续过高，我们可能最终会在最小值附近弹跳，从而无法达到最优解。《小批量随机梯度下降》比较详细地讨论了这一点，在《随机梯度下降》中我们则分析了性能保证。简而言之，我们希望速率衰减，但要比$$\mathcal{O}(t^{-\frac{1}{2}})$$慢，这样能成为解决凸问题的不错选择。
* 另一个同样重要的方面是初始化。这既涉及参数最初的设置方式（详情请参阅《数值稳定性和模型初始化》），又关系到它们最初的演变方式。这被戏称为*预热*（warmup），即我们最初开始向着解决方案迈进的速度有多快。一开始的大步可能没有好处，特别是因为最初的参数集是随机的。最初的更新方向可能也是毫无意义的。
...
此外，我们混合网络以提高性能。
由于大多数代码都是标准的，我们只介绍基础知识，而不做进一步的详细讨论。如果需要，请参阅《卷积神经网络》进行复习。
...
![](../img/output_lr-scheduler_1dfeb6_21_1.svg)
...
![](../img/output_lr-scheduler_1dfeb6_53_0.svg)
...
![](../img/output_lr-scheduler_1dfeb6_68_1.svg)
...
![](../img/output_lr-scheduler_1dfeb6_83_0.svg)
...
![](../img/output_lr-scheduler_1dfeb6_98_0.svg)
...
![](../img/output_lr-scheduler_1dfeb6_113_1.svg)
...
![](../img/output_lr-scheduler_1dfeb6_128_0.svg)
...
![](../img/output_lr-scheduler_1dfeb6_143_1.svg)
...
![](../img/output_lr-scheduler_1dfeb6_158_0.svg)
...
![](../img/output_lr-scheduler_1dfeb6_173_1.svg)

预热可以应用于任何调度器，而不仅仅是余弦。
有关学习率调度的更多实验和更详细讨论，请参阅 `Gotmare.Keskar.Xiong.ea.2018`。
...
1. 预热应该持续多长时间？
2. 可以试着把优化和采样联系起来吗？首先，在随机梯度朗之万动力学上使用 `Welling.Teh.2011`的结果。