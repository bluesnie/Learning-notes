...
$$f(x + \epsilon) = f(x) + \epsilon f'(x) + \mathcal{O}(\epsilon^2).\space\space\space\space\space\text{11.3.1}$$
...
$$f(x - \eta f'(x)) = f(x) - \eta f'^2(x) + \mathcal{O}(\eta^2 f'^2(x)).\space\space\space\space\space\text{11.3.2}$$
...
![](../img/output_gd_79c039_111_1.svg)

## 自适应方法

正如我们在上面学习率中所看到的，选择“恰到好处”的学习率$$\eta$$是很棘手的。
...
$$f(\mathbf{x} + \boldsymbol{\epsilon}) = f(\mathbf{x}) + \boldsymbol{\epsilon}^\top \nabla f(\mathbf{x}) + \frac{1}{2} \boldsymbol{\epsilon}^\top \nabla^2 f(\mathbf{x}) \boldsymbol{\epsilon} + \mathcal{O}(\|\boldsymbol{\epsilon}\|^3).\space\space\space\text{11.3.8}$$
...
毕竟，$$f$$的最小值满足$$\nabla f = 0$$。
遵循中的微积分规则，
通过取$$\boldsymbol{\epsilon}$$对公式(11.3.8)的导数，
再忽略不重要的高阶项，我们便得到
...
![](../img/output_gd_79c039_126_1.svg)
...
有关分析和证明，此算法收敛迅速（请参见`Boyd.Vandenberghe.2004`）。
然而，对深度学习而言，这不太可行。
